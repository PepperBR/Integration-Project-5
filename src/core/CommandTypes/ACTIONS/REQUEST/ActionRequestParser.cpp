...
auto ActionRequestParser::verifyNormal(const std::vector<uint8_t> &data) -> VerifyFrameResponse
{
    ...
    if (data.size() > 12)
    {
        uint8_t hasParams = data[12];
        if (hasParams == 0x00)
        {
            response.fields.push_back({"Method-Invocation-Parameters", 12, 1, "0", "Sem parâmetros de invocação"});
        }
        else if (hasParams == 0x01)
        {
            response.fields.push_back({"Method-Invocation-Parameters", 12, 1, "1", "Parâmetros de invocação presentes"});

            if (data.size() > 13)
            {
                size_t paramLen = data.size() - 13;
                response.fields.push_back({"Data (method parameters)", 13, static_cast<int>(paramLen), std::to_string(paramLen) + " bytes",
                                           "Dados passados ao método COSEM"});
            }
        }
        else
        {
            response.valid = false;
            response.errors.push_back({3, "Indicador de Method-Invocation-Parameters inválido.", ""});
        }
    }
